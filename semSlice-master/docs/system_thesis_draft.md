...
### 2.1 业务需求分析
系统面向“管理员下发配置 + 租户提交任务”的多租户场景，要求在统一资源池约束下实现可解释的语义服务保障。核心需求包括：

1. 支持管理员配置全局网络参数与切片模板。
2. 支持租户按任务类型（高保真/低时延）提交业务请求。
3. 支持语义估计、切片适配、资源分配、性能评估全流程执行。
4. 支持多策略实时切换与对比分析。
5. 支持结果可视化与状态可追踪。
...
1. 认证与权限模块：`/auth/login`、`/auth/logout`、`/auth/me`，区分管理员与租户。
2. 配置模块：`/module/business/config`、`/module/network/config`、`/module/slice/config`。
3. 适配模块：`/module/adaptation`，根据业务特征匹配切片。
4. 编排模块：`/module/resources/allocate`，执行策略相关资源分配。
5. 评估模块：`/module/performance/evaluate`，输出核心指标与图表数据。
6. 系统编排模块：`/system/admin/run`、`/system/tenant/submit` 等端到端运行接口。

### 2.3 非功能需求
系统除功能完整性外，还需满足以下非功能性指标：

1. 可扩展性：模块松耦合，便于替换适配算法或分配策略。
2. 可复现性：保留 legacy 适配层与对比脚本入口。
3. 可维护性：统一数据模型（Pydantic），减少接口歧义。
4. 可观测性：输出 `used_resources`、`remaining_resources`、`timeline` 等运行状态。
5. 低门槛部署：前端静态托管，后端单进程即可运行。

### 2.4 系统总体架构
系统采用前后端分离架构，前端负责配置与可视化，后端负责算法服务与流程编排。按照职责可抽象为 5 层：

1. L0 角色与权限层：管理员/租户认证与访问控制。
2. L1 交互层：Web 控制台、状态展示、图表呈现。
3. L2 API 与流程编排层：统一路由、租户隔离、流程组织。
4. L3 算法服务层：语义处理、切片构建、适配、编排、评估。
5. L4 数据与实验支撑层：状态存储、对比结果、legacy 适配。

系统架构图可引用 `docs/diagrams/system_architecture.png`（可作为“图2-1 系统总体架构图”插入模板）。

### 2.5 关键流程设计
系统运行主流程如下：

1. 管理员登录后下发网络与切片配置。
2. 租户提交业务任务（任务类型、符号长度、距离、词表信息）。
3. 后端执行语义处理，估计 SNR、时延和保真度基线。
4. 适配模块完成“用户-切片”关系映射。
5. 编排模块按策略分配带宽、功率、计算资源。
6. 评估模块计算核心指标并返回图表数据。
7. 前端展示任务明细、资源占用和策略比较结果。

对应流程图可引用 `docs/diagrams/system_flow.png`（可作为“图2-2 系统运行流程图”）。
